...
The particle density at the core of the shower front is highest and then quickly deminishes as the distance increases. In \cref{fig:ldf} the typical particle density on ground versus the core distance is shown for different particle types in the air shower. The most common particles are high energy gamma rays (\SI{>3e9}{\eV}), followed by electrons (\SI{>3e9}{\eV}), then muons (\SI{>3e11}{\eV}). The energy limits are based on detection probabilities and absorption probabilities of the particles, which will be discussed in [todo; reference to section]. The density decreases rapidly with increasing distance. The lateral density distribution can be parameterized by a so called lateral distribution function (LDF). Several such parameterizations exist, the most well known is the Nishimura-Kamata-Greisen (NKG) \cite{apel2006nkg}. This is given by
%
\begin{equation}
    \label{eq:nkg}
...
\begin{figure}
    ...
    \caption{Lateral density distribution. The particle density at ground as a function of core distance for a vertical shower initiated by a proton with $E = \SI{e16}{\eV}$. The contributions from the different particles is shown.}
    \label{fig:ldf}
\end{figure}


\subsection{Shower front profiles in energy and time}
