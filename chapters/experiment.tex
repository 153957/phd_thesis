...
With this the expected detector efficiency for a detector setup can be estimated for various shower energies.
...
By making each station consist of at least two detectors the station can perform triggering as a standalone unit. Because each station performs its own triggering the stations do not need to be interconnected. For detectors separated by large distances this it more practical, since both wireless and wired communication are either to unreliable or to slow.

% Multiple detector readout with ADCs, use the good timing for coincidence time trigger.
Each \pmt is readout with \adcs with a sample rate of \SI{2.5}{\ns}.
