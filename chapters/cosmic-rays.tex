...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]
                    {plots/cosmic-rays/skymap_ta_auger.jpg}
...
The most commonly produced particles in these hadron interactions are light mesons. Charged and neutral pions are most commonly generated. The neutral pions quickly decay ($\tau = \SI{8.4e-8}{\ns}$) via the electromagnetic force into two gamma rays (\SI{98.8}{\percent} of the time). The charged pions decay a bit slower ($\tau = \SI{26}{\ns}$) into muons with same charge, and a muon neutrino. The branching ratio for this is \SI{99.99}{\percent}, the second most probable decay reults in an electron and electron neutrino. Anti particles are created in case of positivly charged pions. The muons may decay if produced at high altiudes and with low enough energy. The mean lifetime of the muon is $\tau = \SI{2197}{\ns}$. Note that the quoted mean lifetimes are for the particle at rest. Given teh relativistic nature of the particles due to the high energies the mean lifetime may be longer in the lab frame.
...
In \cref{fig:schematic_shower} a schematic representation of the hadronic interction is seen. After the first interaction many new particles are created. The leading particle continue towards the ground with a significant part of the energy.
